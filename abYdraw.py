...
def Get_dictionaries(x):
    ...
    if chain_count > 2:
        ...
        for i in range(len(chains)):
            for j in range(len(chains[i])):
                try:
                    ...
                    if ("X" in chains[i][j] or "C[" in chains[i][j]) and len(dicts[i].get(chains[i][j])[0]) == 1 and VHa_VHb_found == False :
        ...
        VLa_match = False
        VLb_match = False
        #print(VHa_keyslist,VHb_keyslist)
        checked_heavy_dicts.append(VHa_checked)
        checked_heavy_dicts.append(VHb_checked)
        VHa_keyslist = list(VHa_checked.keys())
        VHb_keyslist = list(VHb_checked.keys())
...
##Check claw
    ...
    if chain_count > 1:
        for i in range(len(chains)):
            if chains[i] != [] and 'H[' not in str(chains[i]):
                ...
                for x in range(len(note_label_split)):
                    if note_label_split[x] not in str(non_redundant_note_label):
                        if x+1 == len(note_label_split):
                            ...
                            non_redundant_note_label += str(note_label_split[x]+", ")
                if "TYPE:" not in note_label and "NOTE:" not in note_label and "ANTI:" not in note_label and "LENGTH:" not in note_label and "MOD:" not in note_label and "CLASS:" not in note_label:
...
#                    location = dictionary.get(keyslist[i])[0][0]
            ...
            if i == 0:
                ...
                if "H[" in keyslist[i-1] and i-1 != 0:
                ...
                if checker in All_positions_and_chains and ("X" in keyslist[i] or "C[" in keyslist[i]) and len(dictionary.get(keyslist[i])) == 1 and chain_count > 4:
                ...
                elif "H[" in keyslist[i-1] and i-1 == 0:
                    slant = False
                    if righthanded == False:
                        getcoordinates = domainmaker(All_positions_and_chains,(startx+20),(starty+40),righthanded,slant,V,direction,X,mod,interaction,previous_H, Build_up)
                    elif righthanded == True:
                        getcoordinates = domainmaker(All_positions_and_chains,(startx-20),(starty+40),righthanded,slant,V,direction,X,mod,interaction,previous_H, Build_up)
                    ...
                    if chain_count == 1:
                        slant = False
                    if chain_count <=2 and "Linker[" not in keyslist[i]:
                        #if chain_count <=2 and ("H[" not in keyslist[i-1]):# or "L[" in keyslist[i-1]):
                        ...
                        if dictionary.get(keyslist[2]) != [''] and "Linker[" in keyslist[2]:
                            ...
                            if slant == False and "X[" in keyslist[0] and VHa_H_coordinatesx == (width/2)-30:
                                getcoordinates = domainmaker(All_positions_and_chains,(previous_chain[6]),(previous_chain[7]+70),righthanded,slant,V,direction,X,mod,interaction,previous_H, Build_up)

                            elif "H[" in keyslist[i-1] and "Linker[" in keyslist[i-2]:
                                if righthanded == True :
...
##Build up
                        ...
                        if Build_out == True:
                            ...
                            ##Build across
                    elif chain_count==4 and dictionary.get(keyslist[i])[0] != (dictionary.get(previous_domain)[1]) and dictionary.get(keyslist[i])[0] == previous_number and  dictionary.get(keyslist[i])[1]+2 == dictionary.get(keyslist[i-2])[1]:
                        if CLI == False:
                            print("checkpoint16.5")
                            print(dictionary)
                        if dictionary == VLa_chain or dictionary == VLb_chain:
                            tangle_found = False
                            if dictionary == VLa_chain:
                                keyslista = list(VLa_chain.keys())
                                keyslistb = list(VHa_chain.keys())
                                try:
                                    interactor = VLa_chain.get(keyslista[0])[1]
                                    if "Linker[" in keyslista[1]:
                                        interactor_2 = VLa_chain.get(keyslista[2])[1]
                                    else:
                                        interactor_2 = VLa_chain.get(keyslista[1])[1]
                                    if interactor_2 < interactor: #and "H[" in keyslista[-1]:
                                        tangle_found = True
                                except IndexError:
                                    pass
                            elif dictionary == VLb_chain:
                                keyslista = list(VLb_chain.keys())
                                keyslistb = list(VHb_chain.keys())
                                try:
                                    interactor = VLb_chain.get(keyslista[0])[1]
                                    if "Linker[" in keyslista[1]:
                                        interactor_2 = VLb_chain.get(keyslista[2])[1]
                                    else:
                                        interactor_2 = VLb_chain.get(keyslista[1])[1]
                                    if interactor_2 < interactor: #and "H[" in keyslista[-1]:
                                        tangle_found = True
                                except IndexError:
                                    pass
                        ...
                        elif dictionary == VHa_chain or dictionary == VHb_chain:
                            try:
                                if "Linker[" in keyslist[i+1]:
                                    if slant==True and righthanded == True:
                                        getcoordinates = domainmaker(All_positions_and_chains,(previous_chain[0])-45,(previous_chain[1])+95, righthanded,slant,V,direction,X,mod,interaction,previous_H, Build_up)
...
##Build down
                        ...
                        if "H[" not in keyslist[i-2]:
                            ...
                            if clash[0] == True:
                                Build_up = True
                                Build_down=False
                            if "X[" in keyslist[i] and (dictionary == VLa_chain or dictionary == VLb_chain):
                                slant= False
                                Build_up = True
                                Build_down=False
                                if  righthanded == True:
                                    getcoordinates = domainmaker(All_positions_and_chains,(previous_chain[6])-20,(previous_chain[7]+115),righthanded,slant,V,direction,X,mod,interaction,previous_H, Build_up)
                                elif righthanded == False:
                                    getcoordinates = domainmaker(All_positions_and_chains,(previous_chain[6])+20,(previous_chain[7]+115),righthanded,slant,V,direction,X,mod,interaction,previous_H, Build_up)
                                else:
                                    getcoordinates = domainmaker(All_positions_and_chains,(previous_chain[6]),(previous_chain[7]+115),righthanded,slant,V,direction,X,mod,interaction,previous_H, Build_up)
            ...
            if "H[" in keyslist[i] or "H*[" in keyslist[i] or "H^[" in keyslist[i] or "Linker[" in keyslist[i] and len(dictionary.get(keyslist[i])) >1 and (dictionary != VHa_1_test or dictionary != VHb_1_test or dictionary != VLa_1_test or dictionary != VLb_1_test):
                ...
                if Build_up == False:
                    ...
                    if "H" in keyslist[i]:
                        ...
                        disulph_checker = True
                        for n in range(len(list(H_disulphide_coordinates.keys()))):
                            if H_disulphide_coordinates.get(list(H_disulphide_coordinates.keys())[n]) == []:
                                disulph_checker = False
                        if disulph_checker == True:
                            print(interactor,H_disulphide_bridge_count,bottomx,bottomy,topx,topy,righthanded)
                            ...
                            print(H_disulphide_coordinates)
            ...
            if i > 0 and Build_down==True and ("H[" not in keyslist[i]):# and not "Linker[" in keyslist[i]):
                if CLI == False:
                    ...
                    try:
                        if "Linker[" in keyslist[i] and "H[" in keyslist[i+1]:
                            ...
                            #top_bond = (previous_chain[6])-5
                            #bottom_bond = (previous_chain[7]+20)
                            #print(bottom_bond)
                            top_bond = [bottom_bond[0],bottom_bond[1]+30]
                            linkers.append(bottom_bond+ top_bond )
                            l_mod_normal.append(l_mods[-2])
                            bottom_bond = top_bond
                ...
                if "H[" in keyslist[i-1]:
                    if h_mods[-2] !=False:
                        h_mod= h_mods[-2]
                    try:
                        if "Linker" in keyslist[i-2]:
                            bottom_bond[1] = bottom_bond[1] + 30
                    except IndexError:
                        pass
...
##Get labels and positions
            if "H[" not in keyslist[i] and "Linker[" not in keyslist[i] and "X" not in keyslist[i] and "C[" not in keyslist[i]:
                ...
                if x_mod == True:
                    mod_label = "*"
                if "X" in str(keyslist[i]):
                    Domain_name = "X"
                elif "C" in str(keyslist[i]):
                    Domain_name = "C"
                Location_Text.append(str(text)+mod_label)
                Domain_Text.append(str(Domain_name)+mod_label)
...
##VHa_chain
        ...
        if "H[" in str(VHa_1_test) and "H[" in str(VHb_1_test):
            if IgG2 == False:
                ...
                test_keyslist = list(VHa_1_test.keys())
        ...
        if Claw == True:
            ...
            for i in range(len(coordinates_to_change)):
                for j in range(len(coordinates_to_change[i])):
                    if isinstance(coordinates_to_change[i][j], bool) == False:
    ...
    #Labels
        if Label_lock == True:
            for i in range(len(Label_positions)):
                ...
                Domain_Text[i] = re.sub("\_",Negative_charge_character, Domain_Text[i])
                Domain_Text[i] = re.sub("\+",Positive_charge_character, Domain_Text[i])
                Domain_Text[i] = re.sub("nano|nno","",Domain_Text[i])
                label  = canvas.create_text(x,y, text=Domain_Text[i],tags = "label",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
        ...
        if Notes != []:
            ...
            for i in range(len(setlist)):
                if "NOTE:" in setlist[i]:
                    note = canvas.create_text(Note_positions[i],text=setlist[i],tags = "NOTE_labels",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
                    NOTE_labels[note] = [Note_positions[i],setlist[i]]
                elif "TYPE:" in setlist[i]:
                    note = canvas.create_text(Note_positions[i],text=setlist[i],tags = "TYPE_labels",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
                    TYPE_labels[note] = [Note_positions[i],setlist[i]]
                elif "ANTI:" in setlist[i]:
                    note = canvas.create_text(Note_positions[i],text=setlist[i],tags = "ANTI_labels",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
                    ANTI_labels[note] = [Note_positions[i],setlist[i]]
                elif "MOD:" in setlist[i]:
                    note = canvas.create_text(Note_positions[i],text=setlist[i],tags = "MOD_labels",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
                    MOD_labels[note] = [Note_positions[i],setlist[i]]
                elif "LENGTH:" in setlist[i]:
                    note = canvas.create_text(Note_positions[i],text=setlist[i],tags = "LENGTH_labels",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
                                            ...
                                            for g in combinations_to_try:
                                                if d1x1 <= g[0] <= d1x2 and d1y1 <= g[1] <= d1y2:
                                                    ...
                                                    if "*" in str(strings[i][j]):
                                                        ijDomain_name = "H*"
                                                    elif "^" in str(strings[i][j]):
                                                        ijDomain_name = "H^"
                                                    else:
                                                        ijDomain_name = "H"
                                                    if "*" in str(strings[a][b]):
                                                        abDomain_name = "H*"
                                                    elif "^" in str(strings[a][b]):
                                                        abDomain_name = "H^"
                                                    else:
                                                        abDomain_name = "H"
        ...
        for j in range(len(full_chains[i])):
            if strings[i][j] != "-":
                ...
                if stored_comment != "":
                    ...
                    elif "^" in strings[i][j] or "MOD" in str(non_redundant_comments):
                        strings[i][j] = re.sub("\^","", strings[i][j])
                        strings[i][j] = strings[i][j].split("(")[0]+"^("+strings[i][j].split("(")[1]
...
def SelectCommentTypeButton(letter):
    ...
    if CustomLabelLock != letter:
        ...
        if letter == "TYPE":
            ...
            ClassLabelButton.config(fg="red")
...
def CommentLabelButton_function(canvas):
    ...
    if CustomLabelLock == "TYPE":
        ...
        status_label.config(text=entry)
        lower_canvas.config(cursor = "plus")
    elif CustomLabelLock == "CLASS":
        CustomLabelButton.configure(fg="red")
        entry=CustomLabelEntry.get("1.0","end-1c")
        lower_canvas.bind("<Button-1>", mm.place_class_label)
        lower_canvas.bind("<ButtonRelease-1>", mm.place_domain_release)
        status_label.config(text=entry)
        lower_canvas.config(cursor = "plus")
...
class MouseMover():
    ...
    def select(self, event):
        ...
        ANTI_keyslist = list(ANTI_labels.keys())
        LENGTH_keyslist = list(LENGTH_labels.keys())
        CLASS_keyslist = list(CLASS_labels.keys())
        if self.item in label_keyslist:
            ...
            lower_canvas.config(cursor = "fleur")
            return(startcoordinates)
        elif self.item in CLASS_keyslist:
            self.startcoordinates = [xc, yc]
            lower_canvas.config(cursor = "fleur")
            return(startcoordinates)
        if "-" not in str(canvas_polygons.get(self.item)[1]):
            ...
            if x1< xc <x2 and y1 < yc < y2:
                ...
                for i in range(len(label_keyslist)):
                    ...
                    #label_text = re.sub(Positive_charge_character,"+",label_text_test)
                    #label_text = re.sub(Negative_charge_character,"-",label_text_test)
                    ...
                    label_text_test = re.sub("\+",Positive_charge_character,label_text_test)
                    label_text_test = re.sub("\-|\_",Negative_charge_character,label_text_test)
...
#
            ...
            if Label_lock == True:
                ...
                if x2-x1 == 70:
                    if new_coordinates[2] > new_coordinates[0]:
                        ...
                        labelx2+=5
                label_name = temp_label.get(temp_label_key[0])[1]
                label  = lower_canvas.create_text([labelx2,labely2], text = label_name, tags = "label", fill=Font_colour, font=(str(Font_Family),str(Font_size)))
        ...
        if domain_self_item == True:
            ...
            for i in range(len(label_keyslist)):
                ...
                if x1 <= labelx <=x2 and y1 <= labely <= y2:
                    ...
                    label  = lower_canvas.create_text([labelx,labely], text = new_display_name, tags = "label", fill=Font_colour,font=(str(Font_Family),str(Font_size)))
            ...
            if Label_lock == True:
                ...
                for i in range(len(label_keyslist)):
                    ...
                    if x1< labelx <x2 and y1 < labely < y2:
                        del canvas_labels[label_keyslist[i]]
                        lower_canvas.delete(label_keyslist[i])
                        label = lower_canvas.create_text([labelx,labely], text = label_text, tags = "label",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
    ...
    def place_domain(self,event):
        ...
        if Label_lock == True:
            ...
            label  = lower_canvas.create_text(domaincoordinates[3], text = str(domain_name), tags = "label", fill=Font_colour,font=(str(Font_Family),str(Font_size)))
    ...
    def place_type_label(self,event):
        ...
        if Type_list_header.get() != "TYPE":
            label = lower_canvas.create_text(xc,yc, text = Type_list_header.get(), tags = "MOD_labels", fill=Font_colour,font=(str(Font_Family),str(Font_size)))
    ...
    def place_note_label(self,event):
        ...
        if entry != "":
            label = lower_canvas.create_text(xc,yc, text = entry, tags = "NOTE_labels", fill=Font_colour,font=(str(Font_Family),str(Font_size)))
    ...
    def place_mod_label(self,event):
        ...
        if mod_list_header.get() != "MOD":
            label = lower_canvas.create_text(xc,yc, text = mod_list_header.get(), tags = "MOD_labels",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
            MOD_labels[label] = [[xc,yc], mod_list_header.get()]
            update_domain_comments(str(", "+mod_list_header.get()),xc,yc)

    def place_class_label(self,event):
        global CLASS_labels
        ...
        xc = widget.canvasx(event.x); yc = widget.canvasy(event.y)
        entry=CustomLabelEntry.get("1.0","end-1c")
        if class_list_header.get() != "CLASS":
            label = lower_canvas.create_text(xc,yc, text = class_list_header.get(), tags = "CLASS_labels",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
            CLASS_labels[label] = [[xc,yc], class_list_header.get()]
            update_domain_comments(str(", "+class_list_header.get()),xc,yc)
    ...
    def place_anti_label(self,event):
        ...
        if entry != "":
            label = lower_canvas.create_text(xc,yc, text = entry, tags = "ANTI_labels", fill=Font_colour,font=(str(Font_Family),str(Font_size)))
    ...
    def place_length_label(self,event):
        ...
        if entry != "":
            label = lower_canvas.create_text(xc,yc, text = entry, tags = "LENGTH_labels", fill=Font_colour,font=(str(Font_Family),str(Font_size)))
    ...
    def clear_error_message(self,event):
                ...
                if Label_lock == True:
                    ...
                    if min_max[1]-min_max[0] == 70:
                        if domain_coordinates[2] > domain_coordinates[0]:
                            ...
                            labelx+=5
                    label  = lower_canvas.create_text(labelx,labely, text = str(domain_name), tags = "label",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
                    canvas_labels[label] = [[labelx,labely], domain_name]
            elif domain_name == "-" :####
        ...
        for i in range(len(list(TYPE_labels.keys()))):
            label = lower_canvas.create_text(domain_coordinates, text = entry, tags = "TYPE_labels",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
            new_TYPE_dict[label] = [domain_coordinates, entry]
        for i in range(len(list(NOTE_labels.keys()))):
            label = lower_canvas.create_text(domain_coordinates, text = entry, tags = "NOTE_labels",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
            new_NOTE_dict[label] = [domain_coordinates, entry]
        for i in range(len(list(MOD_labels.keys()))):
            label = lower_canvas.create_text(domain_coordinates, text = entry, tags = "MOD_labels",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
            new_MOD_dict[label] = [domain_coordinates, entry]
        for i in range(len(list(ANTI_labels.keys()))):
            label = lower_canvas.create_text(domain_coordinates, text = entry, tags = "ANTI_labels",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
            new_ANTI_dict[label] = [domain_coordinates, entry]
        for i in range(len(list(LENGTH_labels.keys()))):
            label = lower_canvas.create_text(domain_coordinates, text = entry, tags = "LENGTH_labels",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
            new_LENGTH_dict[label][domain_coordinates, entry]
        for i in range(len(list(CLASS_labels.keys()))):
            label = lower_canvas.create_text(domain_coordinates, text = entry, tags = "CLASS_labels",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
            new_CLASS_dict[label][domain_coordinates, entry]
...
if len(sys.argv) < 2:
    ...
    def redo():
        ...
        if Deletes_to_redo != {}:
            ...
            if "-" not in domain_name:
                ...
                LENGTH_to_redo[to_delete] = LENGTH_labels.get(to_delete)
                del LENGTH_labels[to_delete]
            elif to_delete in list(CLASS_labels.keys()):
                lower_canvas.delete(to_delete)
                CLASS_to_redo[to_delete] = CLASS_labels.get(to_delete)
                del CLASS_labels[to_delete]
            Deletes_to_redo= {}
            deleted_polygons={}
        else:
            ...
            elif to_redo in list(CLASS_labels.keys()):
                label = lower_canvas.create_text(domain_coordinates, text = entry, tags = "CLASS_labels",fill=Font_colour,font=(str(Font_Family),str(Font_size)))
                CLASS_labels[label] = [domain_coordinates, domain_name]
                del CLASS_to_redo[to_redo]
    ...
    extra_mods  = ""
    domain_charge=""
    Positive_charge_character = u"\u2295"
    Negative_charge_character = u"\u2296"
    ...
    LengthLabelButton = tk.Button(frame2,text="LEN", bg="#CFCFCF", command=lambda: SelectCommentTypeButton("LENGTH"))
    LengthLabelButton.place(relx = 0.81,rely = 0.61,relheight = 0.1, relwidth= 0.09)
    ClassLabelButton = tk.Button(frame2,text="CLASS", bg="#CFCFCF", command=lambda: SelectCommentTypeButton("CLASS"))
    ClassLabelButton.place(relx = 0.90,rely = 0.61,relheight = 0.1, relwidth= 0.09)
    ...
    b_button.bind("<Button-3>", lambda event:domain_type_button_right_click("b"))
    c_button.bind("<Button-2>", lambda event:domain_type_button_right_click("c"))
    c_button.bind("<Button-3>", lambda event:domain_type_button_right_click("c"))
    d_button.bind("<Button-2>", lambda event:domain_type_button_right_click("d"))
    d_button.bind("<Button-3>", lambda event:domain_type_button_right_click("d"))
    e_button.bind("<Button-2>", lambda event:domain_type_button_right_click("e"))
    e_button.bind("<Button-3>", lambda event:domain_type_button_right_click("e"))
    f_button.bind("<Button-2>", lambda event:domain_type_button_right_click("f"))
    f_button.bind("<Button-3>", lambda event:domain_type_button_right_click("f"))
    g_button.bind("<Button-2>", lambda event:domain_type_button_right_click("g"))
    g_button.bind("<Button-3>", lambda event:domain_type_button_right_click("g"))
    h_button.bind("<Button-2>", lambda event:domain_type_button_right_click("h"))
    ...
    Type_options = ["TYPE","TYPE:ZIPPER", "TYPE:FUSION", "TYPE:OPDM","TYPE:SPDP","TYPE:SMCC","TYPE:OTHER"]
    ...
    Type_menu.place(relx=0.01,rely = 0.83, relwidth=0.22,relheight=0.15)
    ...
    Mod_menu.place(relx=0.23,rely = 0.83, relwidth=0.22,relheight=0.15)
    Class_options = ["CLASS", "CLASS:IgG","CLASS:IgE","CLASS:IgA","CLASS:IgM", "CLASS:IgD","CLASS:OTHER"]
    class_list_header = tk.StringVar(frame2)
    class_list_header.set(Class_options[0])
    class_menu = tk.OptionMenu(frame2, class_list_header, *Class_options)
    class_menu.place(relx=0.45,rely = 0.83, relwidth=0.22,relheight=0.15)
    ...
    "Triplebody":"VH.a(1:7)-CH1(2:8)-L(3)-VL.b(4:6)-L(5)-VH.b(6:4)|VL.a(7:1)-CL(8:2)-L(9)-VL.c(10:12)-L(11)-VH.c(12:10)",
    ...
    "Fab-IgG":"VH.a(1:21)-CH1(2:22){1}-L(3)-VH.a(4:9)-CH1(5:10){1}-H(6:16){2}-CH2(7:17)-CH3(8:18)|VL.a(9:4)-CL(10:5){1}|VH.a(11:23)-CH1(12:24){1}-L(13)-VH.a(14:19)-CH1(15:20){1}-H(16:6){2}-CH2(17:7)-CH3(18:8)|VL.a(19:14)-CL(20:15){1}|VL.a(21:1)-CL(22:2){1}|VL.a(23:11)-CL(24:12){1}",
    ...
    ##Frame 2
        ...
        font_changecolourbutton = tk.Button(text_settings_frame, font=20, text = "Change colour", command =lambda: text_browse_colour())
        font_changecolourbutton.place(relx=0.25, rely = 0.5,relheight = 0.1, relwidth = 0.5)
        ...
        def text_browse_colour():
            global text_new_colour
            text_new_colour = colorchooser.askcolor()[1]
    ...
    comments_buttons = [AntiLabelButton,TypeLabelButton,NoteLabelButton,LengthLabelButton,ModLabelButton, CustomLabelButton,ClassLabelButton]
    ...
    tite = root.title(str('abYdraw'+version_number))
